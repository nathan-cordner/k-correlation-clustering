...
public class RunKVoteLS {
    ...
    public static void main(String args[]) {
        ...
        int[] k_vals = {100, 1000, 10000, 100000}; // {10000, 20000, 30000, 40000, 50000, 60000, 70000, 80000, 90000, 100000, 110000, 120000, 130000, 140000, 150000};
	...
	long hybridScoreTotal = 0;
        ...
        ArrayList<ArrayList<Integer>> pivot_result = DNode.k_random_node_network(prob_matrix, k);
        ...
        ArrayList<ArrayList<Integer>> fill_result = KLocalSearch.k_local_search_network3(prob_matrix, copy1, k, 1.0);
        ...
        ArrayList<ArrayList<Integer>> rand_result = KLocalSearch.k_local_search_network3(prob_matrix, copy2, k, 5.0);
        ...
        ArrayList<ArrayList<Integer>> blend_result = KLocalSearch.k_local_search_network3(prob_matrix, copy3, k, 10.0);
        long blendTime = System.currentTimeMillis() - blendStart;
        blendTimeTotal += (blendTime / 1000.0); 
        blendTimes[j] = blendTime;

        ArrayList<ArrayList<Integer>> copy4 = new ArrayList<ArrayList<Integer>>();
        for (int i = 0; i < pivot_result.size(); i++) {
            ArrayList<Integer> cluster_copy = new ArrayList<Integer>();
            cluster_copy.addAll(pivot_result.get(i));
            copy4.add(cluster_copy);
        }       

        long hybridStart = System.currentTimeMillis();
        ArrayList<ArrayList<Integer>> fix = pivot_result; // KLocalSearch.k_local_search_network3(prob_matrix, copy4, k, -1);
